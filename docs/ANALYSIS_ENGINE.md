...
当前实现的逐项数学定义见 [技术分析公式汇总](TECHNICAL_FORMULAS.md)；基本面财务
量化、Layer3权重、否决和排名公式见
[基本面分析公式汇总](FUNDAMENTAL_FORMULAS.md)。
...
研究结果必须带运行编号、截止日期、输入指纹和研究版本元数据。v2还要求每个关键
结论映射到证据编号，并显式记录单位、低基数、并表口径、未完成计划、来源冲突等
质量问题；任一边界不匹配都会拒绝对应股票的导入。
